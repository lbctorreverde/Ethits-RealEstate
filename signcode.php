...
if (isset($_POST["btn_registerAgent"])){
    $email = $_POST["email"];
    $query=$connect->query("SELECT `email` FROM `tbl_agent` WHERE `email` = '$email'");
    if($query->num_rows) {?>
        <script>
...
if (isset($_POST["btn_registerUser"])){
    $email = $_POST["email"];
    $query=$connect->query("SELECT `email` FROM `tbl_user` WHERE `email` = '$email'");
    if($query->num_rows) {?>
        <script>
            alert('Email is already being used');
            location = 'signupUser.php';
            exit;
        </script>
        <?php
    }else{
